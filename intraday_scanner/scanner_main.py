...
# Define project root at the top level (go up from intraday_scanner/ to project root)
...
class MarketScanner:
    ...
    def _load_symbol_universe(self) -> List[str]:
        ...
        if os.path.exists(intraday_config_path):
            try:
                with open(intraday_config_path, "r", encoding="utf-8") as handle:
                    ...
                    try:
                        ...
                        logger.debug(f"⚠️ InstrumentMapper import failed: {import_err}, using fallback symbol loading")
            ...
            logger.debug("⚠️ Intraday crawler config not found at %s, using volume_averages_20d.json fallback", intraday_config_path)
            # Fallback to bucket_incremental_volume averages
            volume_file = os.path.join(project_root, "config", "volume_averages_20d.json")
            if os.path.exists(volume_file):
                try:
    ...
    def _send_individual_alert(self, pattern):
        ...
        try:
            ...
            try:
                ...
                if result:
                    # Check if alert was actually sent or just grouped
                    # We can't easily determine this from result, so check grouping logs
                    logger.info(f"✅ [ALERT_DEBUG] ALERT PROCESSED: {symbol} {pattern_name} (check grouping status in alert_manager logs)")
                    # Set Redis cooldown after successful send (only if actually sent)
                    # Note: Cooldown should be set AFTER actual notification, not here
